...
import { Book, Globe, Plus, Trash2, FileText, BookOpen, Heart, Save, X, Edit } from 'lucide-react';
...
const EditArticlePage: React.FC<EditArticlePageProps> = ({ language, setLanguage }) => {
  const navigate = useNavigate();
  const { articleId } = useParams<{ articleId: string }>();
  const { trackButtonClick } = useGoogleAnalytics();
  const { article: existingArticle, loading: articleLoading, error: articleError } = useArticle(articleId);
  ...
  // Load existing article data when it becomes available
  useEffect(() => {
  ...
  const addContentSection = (contentType: string, lang: Language) => {
    ...
    trackButtonClick('add_content_section_edit', {
  ...
  const removeContentSection = (index: number, lang: Language) => {
    ...
    trackButtonClick('remove_content_section_edit', {
      language: lang,
      section_index: index,
      article_id: articleId
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackButtonClick('article_updated', {
        article_id: articleId,
        theme: article.theme,
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to update article');
    } finally {
      ...
      <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50 to-orange-50 flex items-center justify-center">
        <div className="text-center">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
            <p className="text-red-700 mb-4">{articleError}</p>
            <Link
              to="/"
            ...
            >
              {language === 'tamil' ? 'முகப்புக்குத் திரும்பு' : 'Back to Home'}
            </Link>
          </div>
        </div>
      </div>
          ...
          <div className="flex justify-between items-center h-16">
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className={`text-xl font-semibold text-gray-800 ${getFontClass()}`}>
                {language === 'tamil' ? 'கட்டுரையைத் திருத்து' : 'Edit Article'}
              </span>
            </Link>
            ...
            <div className="flex items-center space-x-4">
              <button
                onClick={() => {
                  trackButtonClick('language_toggle_edit', { 
              ...
              <Link
                ...
                onClick={() => trackButtonClick('cancel_edit_article', { article_id: articleId })}
              >
                <X className="h-6 w-6" />
              </Link>
            </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl overflow-hidden">
          <div className="p-8">
            <h1 className={`text-3xl font-light mb-8 text-center text-gray-800 ${getFontClass()}`}>
              {language === 'tamil' ? 'கட்டுரையைத் திருத்தவும்' : 'Edit Article'}
            </h1>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={article.id}
                    className={`w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 text-gray-500 ${getFontClass()}`}
                    readOnly
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    {language === 'tamil' ? 'கட்டுரை ஐடியை மாற்ற முடியாது' : 'Article ID cannot be changed'}
                  </p>
                </div>
                ...
                >
                  {isSubmitting ? (
                    <>
                      ...
                      <span>{language === 'tamil' ? 'புதுப்பிக்கப்படுகிறது...' : 'Updating...'}</span>
                    </>
                  ) : (
                    <>
                      <Edit className="h-5 w-5" />
                      <span>{language === 'tamil' ? 'கட்டுரையைப் புதுப்பிக்கவும்' : 'Update Article'}</span>
                    </>
                  )}
                </button>
              </div>
